...
Alice,Chen,alice@initech.com,,Initech,qualified,cold_outreach,25000,decision_maker,immediate,San Francisco,USA,
`;
...
export default function LeadImportModal({ onDone, onClose }: Props) {
  ...
  const downloadSample = () => {
    ...
    a.href = url; a.download = 'nucrm_leads_import_sample.csv';
  ...
  const runImport = async () => {
    setImporting(true);
    try {
      const res = await fetch('/api/tenant/leads/import', {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div>
            <h2 className="font-bold text-base">Import Leads</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border p-4 space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {['first_name *', 'last_name', 'email', 'phone', 'mobile', 'company_name', 'lead_status', 'lead_source', 'budget', 'authority_level', 'timeline', 'lifecycle_stage', 'score', 'city', 'country', 'website', 'linkedin_url', 'twitter_handle', 'tags', 'notes', 'utm_source', 'utm_campaign'].map(col => (
              ...
              <div className="flex gap-4 flex-wrap">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Skip leads with matching email</p>
                  </div>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Overwrite matching leads</p>
                  </div>
                </label>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={m.label} className="p-4 text-center rounded-xl border border-border bg-card">
          ...
          <div className="flex gap-2">
            {step === 'preview' && (
              <button onClick={runImport} disabled={importing}
                ...
                {importing ? `Importing ${totalLines} rows...` : `Import ${totalLines} leads`}
              </button>
            )}
            {step === 'result' && (
